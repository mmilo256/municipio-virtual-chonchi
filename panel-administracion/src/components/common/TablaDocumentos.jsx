  ...
  useEffect(() => {
    (async () => {
      ...
      const formattedData = data.map((doc) => ({
        nombre: (
          <a target="__blank" href={`${API_URL}/${doc.ruta}`} className="text-blue-600 underline">
            {doc.nombre}
          </a>
        ),
        acciones: (
          <button
            onClick={() => {
              handleDeleteDocument(id, doc.id);
            }}
            className="text-red-800 text-sm bg-red-200 hover:bg-red-300 hover:cursor-pointer px-4 rounded"
          >
            Borrar
          </button>
        ),
