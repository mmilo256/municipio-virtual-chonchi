...
const EnviarDocumento = () => {
  ...
  const agregarDestinatario = (e) => {
    ...
    if (selectedEmail === '') {
      ...
      setDestinatarios((prev) => [...prev, selectedEmail]);
      setSelectedEmail('');
    }
  };
  ...
  const enviarCorreo = async () => {
    ...
    try {
      await sendEmail(
        ...
        approveTemplate(userFullName),
        { path: decretoPath, filename: 'decreto.pdf' },
      );
    ...
    <div className="max-w-[60rem] mx-auto bg-[#fff] p-6 pt-0 mt-4 rounded border">
      ...
      <div className="mt-4">
        {destinatarios.map((dest, index) => (
          <span
            key={index}
            className="inline-block py-1 px-2 mr-2 mb-2 bg-green-200 hover:bg-green-300 rounded"
          >
