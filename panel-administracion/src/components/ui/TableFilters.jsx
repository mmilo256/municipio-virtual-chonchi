...
const TableFilters = ({ currentFilters, setCurrentFilters, setCurrentPage }) => {
  ...
  const activeButtonStyles =
    'block rounded-full py-1 px-4 bg-slate-500 border hover:bg-slate-700 border-slate-500 hover:border-slate-700 text-white';
  const inactiveButtonStyles =
    'block rounded-full py-1 px-4 hover:text-slate-700 hover:border-slate-500 text-slate-600 border border-slate-300';
    ...
    <>
      ...
      <div className="flex gap-2 mb-2">
        <button
          ...
          className={`block rounded-full py-1 px-4 ${btnPendiente ? activeButtonStyles : inactiveButtonStyles}`}
        >
          Pendientes
        </button>
        <button
          onClick={() => {
            onFilter('en revision');
          }}
          className={`block rounded-full py-1 px-4 ${btnEnRevision ? activeButtonStyles : inactiveButtonStyles}`}
        >
          En revisión
        </button>
        <button
        ...
        <button
          ...
          className={`block rounded-full py-1 px-4 ${btnAprobada ? activeButtonStyles : inactiveButtonStyles}`}
        >
          Aprobadas
        </button>
        <button
